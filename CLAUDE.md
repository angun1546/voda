...
| **코딩 방식** | Gemini CLI 바이브 코딩 (자연어 → 코드 생성) |
...
```
Google Stitch (자연어 UI 생성)
  → Figma (내보내기·다듬기)
  → Gemini CLI (React 코드 생성)
  → Render (배포)
```
...
| 기술 | 용도 |
|------|------|
| Google Stitch | 자연어 기반 UI 아이데이션 |
| Figma | 디자인 시스템 |
| Gemini CLI | Git 연동·바이브 코딩 |
| Render | 프론트엔드·백엔드 배포 |
...
> 미커밋 변경사항이 있는 상태에서 브랜치를 전환하면 오류가 발생한다.
> **반드시 커밋 후 브랜치를 전환한다.**

### README.md 관리

- `CLAUDE.md` 내용을 기반으로 작성한다
- 포함 항목: 프로젝트 개요, 기술 스택, 프로젝트 구조, 시작 방법